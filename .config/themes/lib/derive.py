...
def do_kdeglobals(colors):
    # ~/.config/kdedefaults/kdeglobals pins General/ColorScheme=BreezeDark as a
    # session default. KConfig only overrides a key the user's own kdeglobals
    # actually SETS - and nothing here ever set this one - so that default
    # silently won out for anything that resolves colour by named scheme
    # rather than by reading the Colors:* groups directly. Measured: Dolphin's
    # Places sidebar (#3daee9, BreezeDark's own Selection colour) stayed
    # Breeze-blue even after every Colors:Selection value below was correct.
    # An empty value still counts as "set" and masks the default layer.
    print("General|ColorScheme|")
...
# ── rofi ──────────────────────────────────────────────────────────────────────
# rofi's .rasi format has its own "@name" variables - a different thing from
# GTK's @define-color despite the look-alike syntax - so it can't @import
# palette.css directly. shared/colors.rasi used to @import a fixed
# colors/onedark.rasi instead, which is why rofi kept a dark blue-grey
# background no matter what the rest of the desktop was themed to.
#
# Names/roles match colors/onedark.rasi so this is a drop-in replacement for
# it - style-1.rasi only actually reads @background-alt/@foreground/@selected
# today, but @background/@active/@urgent are generated too for any rasi that
# expects the full onedark-style set.
ROFI_MAP = [
    ("background",     ("theme_bg_color",)),
    ("background-alt", ("card_bg_color", "theme_base_color")),
...
def do_rofi(colors):
    print("/* GENERATED by ~/.config/themes/lib/derive.py from the theme pack's")
    print(" * palette.css - do not edit. */")
    print()
    print("* {")
    for key, names in ROFI_MAP:
        rgb = pal.first(colors, *names)
        if rgb:
            print(f"    {key}:{' ' * max(1, 15 - len(key))}{pal.hex6(rgb)}FF;")
    print("}")
